...
export default function LeadChatbot() {
  ...
  const [form, setForm] = useState({ name: "", mobile: "", email: "", goal: goals[0], preferredTime: "Morning", website: "" });
  if (["/app", "/admin", "/login"].some(path => pathname.startsWith(path))) return null;

  const submit = async (event: FormEvent) => {
    event.preventDefault(); setBusy(true); setError("");
    try { const response = await fetch("/api/chat-lead", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); setSent(true); }
