...
export default function InstitutionEnquiry() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setError("");
    try {
      const response = await fetch("/api/institution-enquiry", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error);
      setSent(true);
      const participants = Number(form.participantCount);
      trackConversion("college_proposal_submitted", { programme: form.topic, participant_band: participants >= 200 ? "200_plus" : participants >= 100 ? "100_199" : participants >= 50 ? "50_99" : "under_50" });
    ...
    <section id="institutions" className="scroll-mt-32 bg-[#062B5C] px-5 py-16 text-white sm:px-8 sm:py-20 lg:px-12">
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[.85fr_1.15fr] lg:items-start">
        <div>
          ...
          <p className="mt-5 text-lg leading-8 text-blue-100">Start with five essential details. We will collect timetable and delivery preferences during the follow-up conversation.</p>
          <div className="mt-8 space-y-4">
            {[[Presentation, "Fifteen training days with flexible scheduling"], [Users, "Cohort practice for presentations, GDs and interviews"], [Building2, "Pre/post insight with practical mentor guidance"]].map(([Icon, text]) => {
              const FeatureIcon = Icon as typeof Presentation;
              return <div key={text as string} className="flex items-center gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-white/10"><FeatureIcon className="text-[#00D97E]"/></span><p className="font-semibold">{text as string}</p></div>;
            })}
          </div>
        </div>
        ...
        <div className="rounded-[2rem] bg-white p-6 text-[#062B5C] shadow-2xl sm:p-8">
          {sent ? (
            <div className="py-10 text-center">
              ...
              <p className="mt-3 leading-7 text-slate-600">Your enquiry has been received. We will contact you to understand the cohort and discuss the most practical pilot.</p>
            ...
            <form onSubmit={submit}>
              ...
              <p className="mt-2 text-sm text-slate-500">Only the essentials are required. Official email is optional.</p>
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                ...
                <Field label="Role or designation" value={form.designation} onChange={(value) => setForm({ ...form, designation: value })}/>
                <Field label="Mobile / WhatsApp" type="tel" value={form.mobile} onChange={(value) => setForm({ ...form, mobile: value })}/>
                <Field label="Approximate student count" type="number" value={form.participantCount} onChange={(value) => setForm({ ...form, participantCount: value })}/>
                <Field label="Official email (optional)" type="email" required={false} value={form.email} onChange={(value) => setForm({ ...form, email: value })}/>
              </div>
              <Select label="Primary programme interest" value={form.topic} options={topics} onChange={(value) => setForm({ ...form, topic: value })}/>
              ...
              <button disabled={busy} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-[#00D97E] px-5 py-4 font-extrabold disabled:opacity-50">{busy ? "Sending..." : "Request the 15-day bootcamp proposal"}<Send size={18}/></button>
...
function Select({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (value: string) => void }) {
  return <label className="mt-4 block text-sm font-bold">{label}<select value={value} onChange={(event) => onChange(event.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 font-normal">{options.map((option) => <option key={option}>{option}</option>)}</select></label>;
}
